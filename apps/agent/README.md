...
- Node.js **18+** (`node --version`). That's the only requirement.

## Run it

Copy `agent.mjs` to the machine (or this whole folder) and run:

```bash
node agent.mjs
# custom port / token:
PORT=4127 AGENT_TOKEN=your-shared-secret node agent.mjs
```
...
<plist version="1.0"><dict>
  ...
  <array><string>/usr/local/bin/node</string><string>/path/to/agent.mjs</string></array>
...
[Service]
ExecStart=/usr/bin/node /opt/ai-orchestrator/agent.mjs
...
**Windows** — run at startup via Task Scheduler (`node C:\path\to\agent.mjs`) or a service wrapper
